...
def options(opt):
    opt.add_option('--enable-eigen', action='store_true', default=False, help='Enable Eigen library for nr MIMO features')
    opt.add_option('--enable-sqlite', action='store_true', default=False, help='Enable SQLite-dependent examples')

def configure(conf):
    
    conf.check_cfg(package='sqlite3', uselib_store='sqlite3', mandatory=False)
    conf.load('compiler_cxx')
    if conf.options.enable_eigen:
        conf.check_cfg(package='eigen3', args=['--cflags', '--libs'], uselib_store='EIGEN', mandatory=True)
        
    else:
        print("WARNING: nr MIMO features require the eigen3 library, but it was not found")


def build(bld):
    eigen_sources = 'model/nr-mimo-matrices-eigen.cc' if bld.env.ENABLE_EIGEN else 'model/nr-mimo-matrices-no-eigen.cc'
    module = bld.create_ns3_module('nr', ['internet-apps','flow-monitor',
                 'spectrum',
                 'buildings', 
                 'virtual-net-device',
                 'point-to-point',
